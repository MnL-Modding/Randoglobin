...
1. Download the binary from [Releases](https://github.com/MnL-Modding/Randoglobin/releases) and run it. (Use the `.exe` for Windows, and the `.bin` for Linux)
     - Alternatively, download the latest nightly build for [Windows](https://nightly.link/MnL-Modding/Randoglobin/workflows/build/main/Windows%20build.zip) or [Linux](https://nightly.link/MnL-Modding/Randoglobin/workflows/build/main/Linux%20build.zip) and run that instead. This will give you the latest features when they're implemented, but stability isn't guaranteed.
